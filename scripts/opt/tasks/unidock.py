...
class UniDockTask(BaseDockingTask):
    ...
    def run_localopt(self, mols: list[Chem.Mol]) -> list[float]:
        raise NotImplementedError("Redocking is not implemented for UniDockVinaTask")
...
class UniDockVinaTrainer(RxnFlow3DTrainer_single[UniDockTask]):
    def setup_task(self):
        self.task = UniDockTask(cfg=self.cfg)
...
class UniDockVinaMOOTrainer(RxnFlow3DTrainer_single[UniDockMOOTask]):
    def setup_task(self):
        self.task = UniDockMOOTask(cfg=self.cfg)
